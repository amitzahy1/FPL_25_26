        ...
        const statsHtml = `
            <div class="rec-stats-comparison">
                <div class="rec-stat-box">
                    ...
                    <div class="rec-diff ${'diff-pos' if diffPred >= 0 else 'diff-neg'}">
                        ${'+' if diffPred >= 0 else ''}${diffPred.toFixed(1)}
                    </div>
                </div>
                <div class="rec-stat-box">
                    <div class="rec-stat-label">נקודות GW אחרון</div>
                    <div class="rec-stat-value">${recStats.lastGw}</div>
                    <div class="rec-diff ${'diff-pos' if diffLast >= 0 else 'diff-neg'}">
                        ${'+' if diffLast >= 0 else ''}${diffLast}
                    </div>
                </div>
                <div class="rec-stat-box">
                    <div class="rec-stat-label">PPG/90 משוקלל</div>
                    <div class="rec-stat-value">${recStats.ppg90.toFixed(2)}</div>
                    <div class="rec-diff ${'diff-pos' if diffPpg >= 0 else 'diff-neg'}">
                        ${'+' if diffPpg >= 0 else ''}${diffPpg.toFixed(2)}
                    </div>
                </div>
            </div>
